...
def Plot(mean, label, prj, lat0, long0, color_map, parallels, meridians, global_pr, S_N):
 ...
 for n in range (0, 4): 
  ...
  for i in range(0,mean[n].shape[0]):
   ...
   ax = fig.add_subplot(str(n_r), str(n_c), i+1) #n_of_row, n_of_columns and plot_id 
   ax=plt.gca()
   ...
   if S_N == 1: 
    ...
    contours=bmap.contourf(x,y, mean[n][i].data*-1, cmap=color_map ) 
   bmap.drawcoastlines()
 ...
 if S_N == 1: 
  ...
  #orig_cmap=matplotlib.cm.RdYlBu_r
  #shifted_cmap=shiftedColorMap(orig_cmap, midpoint=0.28, name='shifted')
  #levels=40
  contours=bmap.contourf(x,y, mean[4].data*-1, levels,  cmap=color_map) #levels, cmap='shifted') 
 bmap.drawcoastlines()
 bmap.drawparallels(parallels)
 bmap.drawmeridians(meridians)
...
AR_cubes=iris.load('/gws/nopw/j04/pmip4_vol1/users/vittoria/seaice_annual_uba937_*.nc',  'ice area  (aggregate)' ) 
MO=iris.load_cube('/group_workspaces/jasmin4/bas_palaeoclim/vittoria/seaice_annual_uar766_18502050.nc',     'ice area  (aggregate)')
#concatenate cubes in one single dataset to plot
iris.util.unify_time_units(AR_cubes)
AR=AR_cubes.concatenate_cube()
...
'''
#load MSLP data 
MO=iris.load_cube('/group_workspaces/jasmin4/bas_palaeoclim/vittoria/uar766_SLP_U_V_18502050.nc',  'air_pressure_at_sea_level' ) 
AR=iris.load_cube('/gws/nopw/j04/pmip4_vol1/users/vittoria/uba937_SLP_U_V_18502050.nc',     'air_pressure_at_sea_level')
'''
...
U_MO=iris.load_cube('/group_workspaces/jasmin4/bas_palaeoclim/vittoria/uar766_SLP_U_V_18502050.nc',  'x_wind' ) 
U_AR=iris.load_cube('/gws/nopw/j04/pmip4_vol1/users/vittoria/uba937_SLP_U_V_18502050.nc',     'x_wind')
V_MO=iris.load_cube('/group_workspaces/jasmin4/bas_palaeoclim/vittoria/uar766_SLP_U_V_18502050.nc',  'y_wind' ) 
V_AR=iris.load_cube('/gws/nopw/j04/pmip4_vol1/users/vittoria/uba937_SLP_U_V_18502050.nc',     'y_wind')
...
#Plot(mean, label='SAT (DegC)', prj = 'ortho', lat0 = -90, long0 = 0, color_map='RdYlBu_r', parallels= np.arange(-80.,81.,20.), meridians = np.arange(-180.,181.,20.), global_pr=0, S_N = 0 )
...
##plot S/N SST SH
#Plot(S_to_N, label='SNR of SIC', prj = 'ortho', lat0 = -90, long0 = 0, color_map='YlGnBu', parallels= np.arange(-80.,81.,20.), meridians = np.arange(-180.,181.,20.), global_pr=0, S_N = 1 )
